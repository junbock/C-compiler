...
char *type_kind_name[]={"NULL","ENUM","ARRAY","FUNC","POINTER","VOID"};
...
char *id_kind_name[]={"NULL","VAR","FUNC","TYPE","ENUM","ENUM_LITERAL"};
char *spec_name[]={"NULL","AUTO","STATIC"};

void prt_A_ID_NAME(A_ID *id, int s)
{
	print_space(s);
	printf("(ID=\"%s\") TYPE:%x KIND:%s SPEC=%s LEV=%d \n", id->name, id->type, id_kind_name[id->kind], spec_name[id->specifier], id->level);
}
